...
namespace Binary_Signal {
	...
	void Binary_Signal::Test_Class (void) noexcept {
		size_now = 0;
	...
	void Binary_Signal::GetSignal (Signal_Parameter& a, int index) const noexcept{
		a.level = -1;
	...
	void Binary_Signal::GetSignal (Signal_Parameter& a, int bin_level, const char str) const noexcept{
		a.level = -1;
		for (int i = 0; i < SIZE; i++) {
	...
	void Binary_Signal::SetSignal (const int bin_level, const char signal_duration) noexcept {
		if (size_now - 1 == SIZE) {
			return;
		}
	...
	void Binary_Signal::DeleteSignal (const int time, const char signal_duration) noexcept {
		int sum = 0;
		int index = -1;
		while (sum < time && index < size_now - 1) {
			index++;
	...
	Binary_Signal Binary_Signal::SignalAddition (const Binary_Signal& to_add) noexcept {
		Binary_Signal a;
		if (size_now + to_add.size_now >= SIZE) {
	...
	Binary_Signal operator + (const Binary_Signal& signal1, const Binary_Signal& signal2) noexcept {
		Binary_Signal a;
